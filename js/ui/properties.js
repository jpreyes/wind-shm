...
// ──────────────────────────────────────────────────────────────────────────────
import { computeFloorCR, computeFloorCM, computeTributaryWeights } from '../solver/diaphragm.js?v=195';
import { localAxes } from '../solver/timoshenko.js?v=195';
...
export class PropertiesPanel {
  ...
  async _designCodeSelectorHTML() {
    try {
      const mod = this._designMod || (this._designMod = await import('../design/diseno.js?v=195'));
      const fams = new Set();
  ...
  renderMaterials() {
    ...
    import('../design/materials_catalog.js?v=195').then(({ MATERIAL_FAMILIES, getMaterialDef }) => {
  ...
  _bindSecDesign(card, sec) {
    ...
    if (catSel) import('../design/profiles.js?v=195').then(({ catalogFamilies, catalogNames, profileToSection }) => {
