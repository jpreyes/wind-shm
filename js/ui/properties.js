...
// ──────────────────────────────────────────────────────────────────────────────
import { computeFloorCR, computeFloorCM, computeTributaryWeights } from '../solver/diaphragm.js?v=193';
import { localAxes } from '../solver/timoshenko.js?v=193';
...
export class PropertiesPanel {
  ...
  async _designCodeSelectorHTML() {
    try {
      const mod = this._designMod || (this._designMod = await import('../design/diseno.js?v=193'));
      const fams = new Set();
  ...
  renderMaterials() {
    ...
    import('../design/materials_catalog.js?v=193').then(({ MATERIAL_FAMILIES, getMaterialDef }) => {
  ...
  _bindSecDesign(card, sec) {
    ...
    if (catSel) import('../design/profiles.js?v=193').then(({ catalogFamilies, catalogNames, profileToSection }) => {
