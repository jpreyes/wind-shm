...
// ──────────────────────────────────────────────────────────────────────────────
import { computeFloorCR, computeFloorCM, computeTributaryWeights } from '../solver/diaphragm.js?v=141';
import { localAxes } from '../solver/timoshenko.js?v=141';
...
export class PropertiesPanel {
  ...
  async _designCodeSelectorHTML() {
    try {
      const mod = this._designMod || (this._designMod = await import('../design/diseno.js?v=141'));
      const fams = new Set();
  ...
  _bindSecDesign(card, sec) {
    ...
    card.querySelector('.sd-calc')?.addEventListener('click', async () => {
      ...
      try {
        const { fromShape } = await import('../design/section_props.js?v=141');
