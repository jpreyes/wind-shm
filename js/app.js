...
import './model/macros/turbine.js?v=212';   // registra el macromodelo «Torre eólica» (wind-shm)
...
<div class="prop-field" style="margin-top:8px">
  ...
  _initResizeHandle() {
    ...
    document.addEventListener('mousemove', e => {
      ...
      // Sólo cambia el ANCHO del panel vía la variable CSS; el grid (4 columnas en
      // PÓRTICO clásico, 5 en ReWind con el árbol) lo arma el CSS. Sobrescribir aquí
      // `grid-template-columns` con 4 columnas rompía el layout de 5 (ReWind).
  ...
  _loadJSON(text, filename, keepResults = false, keepProject = false) {
    try {
      ...
      // Sync unit selector
      const _us = document.getElementById('unit-select'); if (_us) _us.value = this.model.units || 'kN-m';
      const _sb = document.getElementById('sb-units'); if (_sb) _sb.textContent = (this.model.units || 'kN-m').replace('-', ' — ');
    } catch (err) {
