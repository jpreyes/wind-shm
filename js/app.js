...
// ──────────────────────────────────────────────────────────────────────────────
import { Model }           from './model/model.js?v=172';
import { Serializer }      from './model/serializer.js?v=172';
import { Viewport }        from './ui/viewport.js?v=172';
import { PropertiesPanel } from './ui/properties.js?v=172';
import { MenuBar }         from './ui/menu.js?v=172';
import { UndoStack }       from './utils/undo.js?v=172';
import { StaticSolver, ensureDefaultLC }   from './solver/static_solver.js?v=172';
import { Results }                         from './solver/postprocess.js?v=172';
import { areaStress, areaBendingStress, vonMises } from './solver/membrane.js?v=172';
import { ModalSolver }                     from './solver/modal_solver.js?v=172';
import { buildNodeIndex, assembleK, assembleF, getNodeDOFs } from './solver/assembler.js?v=172';
import { assembleSparseGlobal, extractFreeCSR } from './solver/sparse.js?v=172';
import { solveNonlinear, solveNonlinearDC } from './solver/nl_lite.js?v=172';
import { assembleKg } from './solver/geometric.js?v=172';
import { makeFactor } from './solver/linsolve.js?v=172';
import { formFind } from './solver/formfind.js?v=172';
import { ModalResults }                    from './solver/modal_results.js?v=172';
import { modalTimeHistory }                from './solver/timehistory.js?v=172';
import { parseAccelerogram, accStats, scaleToPGA, DEMO_PRESETS, G as GACC } from './solver/accelerograms.js?v=172';
import { SpectrumSolver }                  from './solver/spectrum_solver.js?v=172';
import { StagedSolver }                    from './solver/staged.js?v=172';
import { tendonEquivalentLoads, applyTendon, tendonEcc } from './solver/tendon.js?v=172';
import { buildLane, influenceLine, movingLoadEnvelope, responseReaction, responseSection } from './solver/moving_load.js?v=172';
import { newmarkNonlinear, shearBuilding, rayleighDamping } from './solver/nl_timehistory.js?v=172';
import { checkDrift } from './design/serviceability.js?v=172';
import { seleccionarPerfil, steelCandidates, predimensionar, candidatesForFamily } from './design/autodesign.js?v=172';
import { jointSCWB } from './design/seismic.js?v=172';
import { resolveMaterial } from './design/material_props.js?v=172';
import { resolveSectionProps } from './design/section_props.js?v=172';
import { autoDetectDiaphragms, computeFloorCR, applyDiaphragmConstraints } from './solver/diaphragm.js?v=172';
import { splitElement, splitByLength, discretizeAll, joinElements, intersectarElementos } from './model/discretize.js?v=172';
import { localAxes, stiffnessMatrix, massMatrix, transformMatrix, globalStiffness, applyReleases } from './solver/timoshenko.js?v=172';
import { blockCells, cornerGridIndices } from './model/mesher.js?v=172';
import { coonsGridFromCorners } from './model/mesh_map.js?v=172';
import { meshPolygonIntoModel } from './model/mesh_free.js?v=172';
import { smoothAreasInModel } from './model/mesh_quality.js?v=172';
...
class App {
  ...
  _staticWorkerSolve(K, nDOF, freeDOF, Flist, dense = false) {
    return new Promise((resolve, reject) => {
      let worker;
      try { worker = new Worker(new URL('./solver/static_worker.js?v=172', import.meta.url), { type: 'module' }); }
  ...
  async runModal(opts = {}) {
    ...
    try {
      ...
      const modes = await new Promise((resolve, reject) => {
        const worker = new Worker(new URL('./solver/modal_worker.js?v=172', import.meta.url), { type: 'module' });
  ...
  async runTimeHistory(opts = {}) {
    ...
    try {
      ...
      const rawModes = await new Promise((resolve, reject) => {
        const w = new Worker(new URL('./solver/modal_worker.js?v=172', import.meta.url), { type: 'module' });
  ...
  _thSolveInWorker(modes, ag, dt, zeta) {
    return new Promise((resolve, reject) => {
      let w;
      try { w = new Worker(new URL('./solver/timehistory_worker.js?v=172', import.meta.url), { type: 'module' }); }
      catch (e) {
...
4,100</textarea></div>
  ...
  async predimensionarDialog() {
    ...
    if (document.getElementById('pd-assign').checked) {
      const { profileToSection } = await import('./design/profiles.js?v=172');
      this.snapshot();
  ...
  _solveNLInWorker(kind, opts) {
    return new Promise((resolve, reject) => {
      let worker;
      try {
        worker = new Worker(new URL('./solver/nl_worker.js?v=172', import.meta.url), { type: 'module' });
      } catch (e) {
  ...
  async runBuckling(opts = {}) {
    ...
    try {
      ...
      const rawModes = await new Promise((resolve, reject) => {
        const worker = new Worker(new URL('./solver/buckling_worker.js?v=172', import.meta.url), { type: 'module' });
...
<details style="margin-top:12px">
  ...
  async aplicarOperacionesModelo(ops, ctx = null) {
    ...
    this.snapshot();
    const { aplicarOperaciones } = await import('./model/model_ops.js?v=172');
  ...
  async _generarDesdeFicha(fichaText) {
    ...
    try {
      ...
      const { generarModelo } = await import('../asistente/generador.js?v=172');
  ...
  async generarMemoriaDocx() {
    ...
    try {
      const { Docx } = await import('./io/docx.js?v=172');
