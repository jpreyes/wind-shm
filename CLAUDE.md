...
Every internal import and worker URL carries a query string, e.g. `import { Model } from './model/model.js?v=216'`. This is a **global app version** used to bust browser/SW caches. When you ship a change to shipped JS/CSS, **bump it across all files at once** (current version: **v262**):

```bash
files=$(grep -rl "v=262" --include=*.js --include=*.html js app.html sw.js)
for f in $files; do sed -i 's/v=262/v=263/g' "$f"; done
```
...
`docs/ROADMAP.md` is the live ReWind plan, **unified into 5 executable phases** (all items `R-*`, with effort/dependencies/close criteria): Fase 1 hardening+UX (`R-40*`/`R-36*`, from the internal audit), Fase 2 client-side analytics (`R-34` persistent history, `R-35` health index, `R-26` fleet benchmarking, `R-37` replay, `R-23a` alarms v1), **Fase 3 = current: real-data backend `R-10`** ([plan Frente 4](docs/planes/frente-4-backend-r10.md): ADXL355 + Pi gateways → Mosquitto → InfluxDB/Postgres → WS `/live`; Starlink/CGNAT → on-prem + tunnels), Fase 4 deferred tech close (`R-21` OMA with JP's own ultrametric module → `R-31b`/`R-25`/`R-30`), Fase 5 majors (Electron, SCADA, drivetrain). Detailed plans live in `docs/planes/frente-*.md`. The inherited PÓRTICO/FEM history is **not** here — it lives in the upstream `structweb3d` repo. Update the roadmap when closing items.
