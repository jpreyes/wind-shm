...
- ⬜ **Instrumentación editable: colocar/configurar sensores** `[R-33]`: poder **añadir, mover y configurar sensores** en cada estructura (hoy son fijos: 2 MEMS por turbina + gateway, 4 nodos por torre AT). Editor de instrumentación: tipo (acelerómetro/inclinómetro/galga/temperatura), posición/altura (`yFrac`), y los **sensores del gateway**; alimenta el `DataSource`, el HUD por componente y la pestaña Sensores. *(Depende del modelo de datos de `[R-10]` para persistir la configuración por torre.)*
