...
- 🟡 **Internacionalización ES/EN (multilingüe)** `[R-6]` (v262, Fase 1): `js/shm/i18n.js` — diccionario ES/EN + `t(key)` + idioma en `localStorage` + **conmutador ES/EN** en el menú superior (recarga la app para rehacer los render). **Fase 1 traduce todo el MARCO**: barra de herramientas (labels+tooltips), menú superior (Parque/Datos/Informe + ítems + alertas + «Acerca de»), barra de estado, pestañas del panel (Parque/Selección/Obra/Inspección/SHM/Shadow) y subpestañas SHM (Estado/Señal/Sensores/Fatiga/Avanzado), portada y `<html lang>`. **Fase 2 (pendiente)**: cuerpos de los paneles e informes (inspección, obra, SHM, fatiga, shadow) reutilizando el mismo `t()`. *(El CSV `docs/textos_ui.csv` es el inventario viejo de PÓRTICO; no aplica al marco ReWind.)*
