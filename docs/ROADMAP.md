...
- ⬜ **CRÍTICO — pérdida irreversible del modelo** `[#42]`: tras un pushover-DC sin desplazamientos, "Limpiar resultados" (barra lateral) **no restauró el modelo original**: el modelo quedó sobrescrito (aparentemente por la copia auto-discretizada / un estado anterior) y al **Guardar** se perdió definitivamente. Acciones: (a) "Limpiar resultados" debe **restaurar el modelo original** (`_predisc`) y nunca dejar el discretizado como modelo activo; (b) **nunca** sobrescribir el modelo del usuario con el discretizado interno; (c) antes de un Guardar que sobrescriba, **respaldo/confirmación** (snapshot recuperable). Reproducción: pushover-DC δ→0, luego Limpiar resultados.
...
- ⬜ **BUG rótulas plásticas / pushover-DC — escalado de carga/capacidad** `[#46]`: con carga muy superior a la de fluencia dice **"nadie forma rótulas / carga muy baja"**; el **Mp** se rechaza por "demasiado grande" aunque sea ~1 % del momento de trabajo; el pushover-DC con objetivo **1 m** dice "la carga no produce desplazamientos". Revisar el patrón/escala de carga de referencia, las unidades de Mp y el criterio de parada. Probablemente la causa raíz se solapa con `[#42]` (el modelo evaluado no es el esperado).
- ⬜ **BUG resultados de rótulas no se actualizan** `[#46]`: la pestaña «Rótulas» muestra la **secuencia de la corrida anterior** (no refresca tras volver a correr / cambiar de modelo).
...
1. **G8 `[#42]` — CRÍTICO**: detener la pérdida irreversible de modelo (Limpiar resultados / auto-disc / Guardar). Bloquea cualquier uso serio.
2. **G11 `[#46]`**: bugs de rótulas/pushover (no forma rótulas, Mp/δ rechazados, resultados sin refrescar) — probablemente comparten causa con `[#42]`.
