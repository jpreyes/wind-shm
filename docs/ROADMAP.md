...
| ⬜ `R-41` 🌟 | **Módulo «Calidad de obra»** (v1, fases 5.1–5.6 del [plan Frente 5](planes/frente-5-calidad-obra.md)): reader del «Log protocolos SACYR.xlsx» → JSON canónico (literales originales preservados); **writer de VALORES** (mismas hojas/cabeceras/celdas; fórmulas del original → valores calculados; sin gráficos/estilos — no requeridos) + **diff de información** celda a celda; **derivados en JS** (matriz por WTG, KPIs turnaround) validados contra el archivo; pestaña **«Calidad»** (heatmap protocolos×WTG, drill por torre); integración con Obra/HUD/CMMS (avance real opt-in, ensayos hormigón → `R-31`). *(v2 con backend = sub-fase 3.5b.)* | ~1½ semanas | — | Round-trip sin cambios → 0 diffs de valor; con cambios → difieren solo las celdas esperadas. |
