...
- 🟡 **Internacionalización ES/EN (multilingüe)** `[R-6]` (v262–v265): `js/shm/i18n.js` — diccionario ES/EN + `t(key)` + idioma en `localStorage` + **conmutador ES/EN** en el menú superior (recarga la app para rehacer los render). **Hecho — toda la UI interactiva es bilingüe**: Fase 1 (marco: toolbar, menú superior, barra de estado, pestañas y subpestañas, portada, `<html lang>`); Fase 2a (Selección + panes SHM Estado/Señal/Sensores/Fatiga/Avanzado + clasificación en vivo + rótulo/banner); Fase 2b (Parque + micro-CMMS de Inspección completo); Fase 2c (Obra + gemelo de construcción + Shadow flicker). **Pendiente (Fase 2d)**: los **informes imprimibles** (salud SHM `buildReport`, inspección, avance DPR, certificado de commissioning, flicker) siguen en español. **No se traducen los catálogos de dominio** (tipos/causas/severidades de daño, nombres de componentes) por ser claves de scoring/datos. *(El CSV `docs/textos_ui.csv` es el inventario viejo de PÓRTICO; no aplica.)*
